         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="relative w-full max-w-[420px] rounded-[7px] bg-white p-[30px] shadow-2xl transition-all will-change-transform">
               ...
               <div className="relative mb-5">
                  <span className="pointer-events-none absolute left-0 top-0 flex h-12 w-12 items-center justify-center text-gray-400">
                     <MagnifyingGlassIcon className="text[15]" />
                  </span>
                  <input
                     type="text"
                     value={searchTerm}
               ...
               {/* Location List */}
               <ul className="mt-5 max-h-[287px] list-none overflow-y-auto divide-y divide-black/5 pr-1">
...
function Dropdown({ onOpen, selectedLocation }) {
   return (
      <button
         type="button"
         onClick={onOpen}
         className="flex w-full max-w-[180px] items-center gap-[10px] rounded-[7px] border border-[#d9d9e9] bg-white px-[18px] py-[5px] text-[#3e445a] shadow-[0_2px_4px_rgba(0,0,0,0.05)] text-left hover:border-blue-400 transition-colors"
      >
         <div className="flex flex-col min-w-0 flex-1">
            <span className="block text-[10px] opacity-50 uppercase tracking-wider">
               Your Location
            </span>
            <span className="block truncate text-[13px] font-semibold text-[#202435]">
